...
public class KotlinDocumentStore(
    ...
    public suspend fun getJsonCollection(name: String): JsonCollection {
        ...
        return JsonCollection(
            name = name,
            json = json,
            mutex = getMutexForCollection(name),
            store = store,
    ...
    /**
     * Gets or creates a mutex for a collection.
     */
    public suspend fun getMutexForCollection(name: String): Mutex =
        mutex.withLock { mutexMap.getOrPut(name) { Mutex() } }
...
/**
 * Retrieves a [TypedJsonCollection] with strongly-typed IDs for the specified collection name.
 *
 * This method initializes or retrieves a collection that uses the specified ID type instead of Long.
 * The ID type must be serializable through the store's Json configuration.
 *
 * @param ID The type of identifiers to use in this collection.
 * @param name The name of the collection to retrieve.
 * @return A [TypedJsonCollection] with the specified ID type.
 * @throws IllegalStateException if no serializer can be found for the ID type.
 */
@OptIn(ExperimentalSerializationApi::class)
public suspend inline fun <reified ID : Any> KotlinDocumentStore.getTypedJsonCollection(
    name: String,
): TypedJsonCollection<ID> {
    store.getMap(KotlinDocumentStore.COLLECTIONS).put(name, "1")

    // Try to obtain a serializer for ID from kotlinx.serialization
    val serializer = try {
        serializer<ID>()
    } catch (e: Exception) {
        // Fall back to contextual lookup
        json.serializersModule.getContextual(ID::class)
            ?: throw IllegalStateException(
                "No serializer found for ID type ${ID::class}. " +
                        "Register one in the Json.serializersModule or annotate the type with @Serializable",
                e
            )
    }

    val genMapName = "id_gen_typed:$name"
    return TypedJsonCollection(
        name = name,
        json = json,
        mutex = getMutexForCollection(name),
        store = store,
        indexMap = store.getMap(KotlinDocumentStore.INDEXES_MAP_NAME).asIndexOfIndexes(),
        genIdMap = store.getMap(genMapName).asTypedIdGenerator(serializer, json),
        persistentCollection = store.getMap(name).asTypedCollectionMap(serializer, json),
        idSerializer = serializer,
    )
}

/**
 * Retrieves a [TypedObjectCollection] with strongly-typed IDs for the specified collection name and object type.
 *
 * This method initializes or retrieves a collection that uses the specified ID type and stores
 * objects of type T. Both the ID type and object type must be serializable.
 *
 * @param T The type of objects stored in the collection.
 * @param ID The type of identifiers to use in this collection.
 * @param name The name of the collection to retrieve.
 * @return A [TypedObjectCollection] with the specified ID and object types.
 */
public suspend inline fun <reified T : Any, reified ID : Any> KotlinDocumentStore.getTypedObjectCollection(
    name: String,
): TypedObjectCollection<T, ID> =
    getTypedJsonCollection<ID>(name).toTypedObjectCollection<T, ID>()
...
/**
 * Converts the current TypedJsonCollection into a TypedObjectCollection of the specified type.
 *
 * @param T The type of objects to store in the collection.
 * @param ID The type of identifiers used in this collection.
 * @return A TypedObjectCollection containing objects of type T with IDs of type ID.
 */
public inline fun <reified T : Any, ID : Any> TypedJsonCollection<ID>.toTypedObjectCollection(): TypedObjectCollection<T, ID> =
    TypedObjectCollection(
        typedJsonCollection = this,
        serializer = json.serializersModule.serializer(),
    )
